...
export default function SignIn() {
  ...
  function switchMode(next: 'login' | 'register' | 'otp') {
    setMode(next)
    setError('')
    setInfo('')
    setOtpStep('email')
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {mode === 'otp' && otpStep === 'code' && (
            <input
              ...
              inputMode="numeric"
              maxLength={6}
              className="w-full bg-gray-800 text-white px-4 py-3 rounded-xl border border-gray-700 focus:border-indigo-500 outline-none text-center tracking-widest text-xl font-mono"
              placeholder="000000"
              value={otpCode}
          ...
          >
            {loading ? 'Please wait...'
              : mode === 'otp' && otpStep === 'email' ? 'Send code →'
              : mode === 'otp' && otpStep === 'code' ? 'Verify code →'
              : mode === 'login' ? 'Sign In →'
              : 'Create Account →'}
          </button>
        </form>
        ...
        <div className="mt-5 space-y-2 text-center text-sm text-gray-500">
          ...
          {mode === 'login' && (
            <p>
              ...
              <button className="text-indigo-400 hover:text-indigo-300 font-medium" onClick={() => switchMode('register')}>
                Sign up
              </button>
            </p>
          )}
          {mode === 'register' && (
            <p>
              ...
              <button className="text-indigo-400 hover:text-indigo-300 font-medium" onClick={() => switchMode('login')}>
                Sign in
              </button>
            </p>
          )}
          <p>
            <button onClick={() => navigate('/pricing')} className="hover:text-gray-400 text-xs transition-colors">
              See pricing →
            </button>
          </p>
        </div>
